...
import (
	...
	"database/sql/driver"
	"fmt"
	node2 "github.com/xlzhangkeke/entdemo/examples/edges/o2mrecur/ent/node"
	"github.com/xlzhangkeke/entdemo/examples/edges/o2mrecur/ent/predicate"
	"math"
...
func (nq *NodeQuery) QueryParent() *NodeQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(node2.Table, node2.FieldID, selector),
			sqlgraph.To(node2.Table, node2.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, node2.ParentTable, node2.ParentColumn),
		)
...
func (nq *NodeQuery) QueryChildren() *NodeQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(node2.Table, node2.FieldID, selector),
			sqlgraph.To(node2.Table, node2.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, node2.ChildrenTable, node2.ChildrenColumn),
		)
		fromU = sqlgraph.SetNeighbors(nq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (nq *NodeQuery) First(ctx context.Context) (*Node, error) {
	...
	if len(nodes) == 0 {
		return nil, &NotFoundError{node2.Label}
	}
	return nodes[0], nil
}
...
func (nq *NodeQuery) Only(ctx context.Context) (*Node, error) {
	...
	case 0:
		return nil, &NotFoundError{node2.Label}
	default:
		return nil, &NotSingularError{node2.Label}
	}
}
...
func (nq *NodeQuery) OnlyID(ctx context.Context) (id int, err error) {
	...
	case 0:
		err = &NotFoundError{node2.Label}
	default:
		err = &NotSingularError{node2.Label}
	}
	return
}
...
func (nq *NodeQuery) IDs(ctx context.Context) ([]int, error) {
	var ids []int
	if err := nq.Select(node2.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}
...
func (nq *NodeQuery) loadChildren(ctx context.Context, query *NodeQuery, nodes []*Node, init func(*Node), assign func(*Node, *Node)) error {
	...
	query.Where(predicate.Node(func(s *sql.Selector) {
		s.Where(sql.InValues(node2.ChildrenColumn, fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (nq *NodeQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := nq.fields; len(fields) > 0 {
		...
		_spec.Node.Columns = append(_spec.Node.Columns, node2.FieldID)
		for i := range fields {
			if fields[i] != node2.FieldID {
